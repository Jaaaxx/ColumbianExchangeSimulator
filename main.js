...
window.onload = function() {
    ...
    let ctx;
    let rect;
    let anDroplets = [{'timeLeft': 0, 'maxTime': 0, 'image': 'wheat.png', 'size': [0, 0, 0, 0]}];
    let anBigClick = 0;
    ...
    plrl = function(num, string) {
        ...
        else if (string.substr(1, 4) === "bill")
            return " bills of credit"
        else if (string.substr(1, 5) === "wheat")
            return string;
        return string + "s";
    }
    ...
    function gameLoop() {setInterval(function() {
        let spawnChance = Math.floor(Math.random() * 10_00);
    ...
    function onBigPopsicleClick(clientX, clientY, fake=false) {
        ...
        let time = 1000;
        if (droplets < reqDroplets * dMult)
            anDroplets.push({'timeLeft': time, 'maxTime': time, 'image': 'wheat.png', 'size': drawDroplet(x, y)})
    ...
    function popCanvas() {
        ...
        ctx.font = fontpx * 75 + "px 'UnifrakturCook'"
        ctx.fillStyle = "#34d410";
        let totalPopsString;
        totalPopsString = formatSci(totalPops) + plrl(totalPops, " bill of credit");
        totalTextHeight += quickMeasureHeight(totalPopsString, ctx) + tMarg;
        ctx.fillText(totalPopsString, canvas.width / 2, totalTextHeight);

        ctx.fillStyle = "#fffb8a";
        ctx.font = fontpx * 75 + "px 'UnifrakturCook'"
        let totalDropsString;
        totalDropsString = formatSci(droplets) + " wheat";
        ...
        ctx.fillStyle = "white";
        ctx.font = fontpx * 60 + "px 'UnifrakturCook'"
        ...
        for (let i = 0; i < boosts.length; i++) {
            ...
            let img = images['skull.png'].cloneNode(false);
        ...
        // Big Popsicle
        let img = images["wheat.png"].cloneNode(false);
        ...
        // Bucket
        let gimg = bucketHover ? images["boat-clicked.png"] : images["boat.png"]

        let gscale = canvas.width / gimg.width;
        let gwidth = gimg.width * gscale;
        let gheight = gimg.height * gscale;
        let gx = (canvas.width / 2) - (gwidth / 2);
        let gy = canvas.height - gheight
        ...
        if (currTooltip) {
            ...
            if (u instanceof Upgrade) {
                ...
                ttStrings = [
                    ...
                    ["<hr>"],
                    [false, fontpx * 36 + "px 'Open Sans'", "Each " + u.name.toLowerCase() + " produces<#f1f1f1 " + (u.amount === 0 ? formatSci(u.baseProduction * u.multiplier) : formatSci(u.production * u.multiplier)) + plrl(u.production * u.multiplier, " wheat") + "</> per second.", '#a5a49f'],
                    [false, fontpx * 36 + "px 'Open Sans'", "<#f1f1f1" + u.amount + " " + plrl(u.amount, u.name.toLowerCase()) + "</> producing<#f1f1f1 " + formatSci(u.production * u.amount * u.multiplier) + plrl(u.production * u.amount * u.multiplier, " wheat") + "</> per second.", '#a5a49f'],
                    [true, fontpx * 36 + "px 'Open Sans'", "[<#fff1f1" + (dps !== 0 ? (((u.production*u.amount*u.multiplier)/dps)*100).toFixed(0) : dps) + "%</> of WpS]", '#f1f1f1']
                ];
            } else if (u === "prestige") {
                ttStrings = [
            ...
            if (u instanceof Upgrade || u instanceof Building) {
                ...
                if (u instanceof Building && u.name === "Banana" || u instanceof Upgrade && u.type === "banana")
                    ...
                    flaImg = images["icon-horse.png"].cloneNode(false);
                else if (u instanceof Building && u.name === "Coffee" || u instanceof Upgrade && u.type === "coffee")
                    flaImg = images["icon-coffee.png"].cloneNode(false);
    ...
    function onDocumentMouseDown(event) {
        if (!musicPlaying) {
            let music = sounds['music.mp3'].cloneNode(false);
            music.loop = true;
            music.volume *= 0.3;
            music.play();
            musicPlaying = true;
...
class Building {
    ...
    constructor(baseCost, baseProduction, name, description) {
        ...
        if (this.name === "Banana")
            icon = images['icon-banana.png'].cloneNode(false);
        else if (this.name === "Horse")
            icon = images['icon-horse.png'].cloneNode(false);
        else if (this.name === "Coffee")
            icon = images['icon-coffee.png'].cloneNode(false);
    ...
    determineCost() {
        ...
        this.priceButton.innerHTML = this.priceButton.innerHTML.substr(0, this.priceButton.innerHTML.indexOf("\">") + 1) + "$" + formatSci(this.currentCost);
...
class Upgrade {
    ...
    makeHtml() {
        ...
        switch (this.type) {
            case ("horse"): {
                upgrade = images['icon-horse.png'].cloneNode(false);
                break;
            }
            case ("banana"): {
                upgrade = images['icon-banana.png'].cloneNode(false);
                break;
            }
            case ("bucket"): {
...
class Boost {
    ...
    runBoost() {
        if (this.name === 'loseFood' && !this.isRunning) {
            loseFood += 1;
        }
